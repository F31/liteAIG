...
| A3 | 官方 **Client SDK 第一步已落地**：Go、JavaScript/TypeScript 与 Python SDK 覆盖稳定 Gateway 非流式面、chat streaming、MCP JSON-RPC 与 A2A SendMessage/SendStreamingMessage，并默认 pin API contract version；SDK package gate 与兼容矩阵已接入；**真实 registry 发布链路已验证**：dry-run 全绿、修复 release.yaml 两个真实 bug（dry-run SBOM 解析 registry 不存在镜像、anchore 生成路径不在工作区）后，真实 tag `v0.1.0-test.2` 演练完成 GHCR push + GitHub Release + SBOM 附加（49 packages / SPDX-2.3） | `pkg/liteaig`；`packages/liteaig-js`；`packages/liteaig-python`；`scripts/check-sdks.sh`；`docs/SDK_COMPATIBILITY.md`；`release.yaml` | 后续按发布需要补签名 provenance |
...
| C3 | **网关性能基准已落地**：支持真实 Gateway chat/stream 压测，输出 RPS、P50/P95/P99、TTFT P99、错误率与错误摘要，可用 SLO flag 作 release gate；**Standard 拓扑真实基线已补充**：2 副本 + Postgres/Redis 协调下 chat 合并 1221 RPS / P99 25ms，stream 合并 1149 RPS / TTFT P99 25ms，pod 故障注入验证副本独立服务 | `internal/loadtest`；`cmd/loadbench`；`docs/GATEWAY_BENCHMARK.md` | 后续可按 Standard/Enterprise 拓扑沉淀分档基线与 CI 长跑 |
