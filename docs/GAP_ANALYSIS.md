...
| # | 特性 | 现状 | 推进价值 |
|---|---|---|---|
| A1 | image / audio / rerank / batch / **Gemini-native** 接入 | **全部主要能力已有第一切片**：Gemini-native chat/vision/streaming/tools、rerank、audio transcription、batch create/retrieve/list/cancel 均已通过 Gateway + connector 治理路径；batch retrieve/cancel 使用 `batch_mappings`，batch list 使用 `model` query 扩展路由；batch/files artifact 第一切片已支持 `output_file_id`/`error_file_id` 映射、governed `GET /v1/files/{id}/content` 代理、files upload/list/retrieve/delete 治理路由、redacted `file.access` DomainEvent、batch input upload guardrail scanning 与本地 mapping retention purge foundation | 后续按产品需要补上游 artifact 归档/删除策略与更深文件内容分类/报告 |
...
| C4 | **语义缓存可观测与可调第一步已落地**：FinOps 暴露总体/语义命中率，Console 可通过 config draft 调整语义缓存 enabled/model/threshold；**avoided-cost 估算已补**：缓存命中请求按其 would-be provider token 负载累计 `savedTokens`，并按文档化 blended $2/1M token 报价估算 `cacheSavings`（估算值，非 provider 账单），FinOps 缓存建议带出规避 token/预估金额 | `FinOpsView.cacheHitRate/semanticHitRate/savedTokens/cacheSavings`；Governance Cache panel；`aggregate.QuantifyOptimizationCost` | 后续可接真实价格索引（per-model rate）替换 blended 估算，并补更细分的项目/模型缓存质量视图 |
| C5 | **控制面防护第一步已落地**：Admin API 已有限流，登录/密码重置已有专用 limiter；敏感操作 step-up reauth 已覆盖 guardrail fast-publish、`PUT /api/admin/system/config` 与 tenant suspend/delete | `AdminRateLimiter`；`NewLoginLimiter`；`NewResetRateLimiter`；`Server.requireReauth` | 后续补完整高危操作 reauth 策略矩阵（如 key revoke、federation 批准） |
| C6 | **多租户就绪第一步已落地**：Postgres RLS 已全量铺好，新增 system-scope tenant CRUD Admin API；完整 SaaS system role/租户切换仍归 A4 | `rls.go`；`tenancy.TenantAdmin`；`/api/admin/tenants` | 后续按 A4 补 system role、tenant switch、成员关系与策略继承 |
...
| C8 | **OTLP 完整度第一步已落地**：traces/metrics/logs 均有 opt-in OTLP/HTTP exporter；metrics 已接低基数请求指标，logs 已接安全运维日志；**结构化事件日志桥已接入**：所有持久化 DomainEvent（guardrail.match、file.access、system_config.update、alert.*、outcome 等）在事件出口经 `observability.StructuredEventLog` 白名单过滤路由到 OTLP logs（固定 allowlist + tenant/project/request/event_id 上下文 + kind 派生 severity），best-effort 含失败不阻断；**adminapi 安全事件已并入**：reauth 失败/缺 token 与 admin rate-limit 429 通过 `Server.WithEventSink` 输出 `admin.security.*` 结构化 DomainEvent，复用同一 OTLP 导出 | `observability/otlp.go`；`--otlp-*-endpoint`；`observability/eventlog.go`；`logThenEventSink`；`adminapi/server.go` | 后续可扩展更多低基数 gauges/histograms 与登录失败结构化事件 |
| C9 | **公开 API 兼容契约第一步已落地**：Gateway/Admin API 统一 `LiteAIG-API-Version` 头（当前 `2026-09-09`）、未知版本拒绝、旧版本 Deprecation/Sunset 提示，并有策略文档；**module API 稳定性门禁已接入**：`cmd/api-snapshot` 把 `pkg/liteaig` 等公开包的导出面 snapshot 到 `architecture/exported.json`，`cmd/architecture-test` 对比发现「移除已导出符号」即判为 breaking 违规，新增导出符号允许 | `webkit.APICompatibility`；`docs/API_COMPATIBILITY.md`；`cmd/api-snapshot`；`internal/architecture/api.go` | 后续可补 OpenAPI 规范快照 diff，与 module API 门禁配合 |
