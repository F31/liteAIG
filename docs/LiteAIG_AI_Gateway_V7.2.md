# LiteAIG v7.2 —— 企业级 AI Governance & Agentic Traffic Control Plane

> 版本：v7.2  
> 日期：2026-08-27  
> 状态：Development Baseline / Modular Architecture Frozen  
> 目标：构建面向 Model / Tool / Agent 的统一企业 AI 治理与流量控制平面。采用模块化单体、单二进制优先架构；通过稳定 Kernel、协议适配、连接器、治理域和控制面边界保证可演进性，同时支持 Agent Identity、Delegation、Task、MCP、A2A、多智能体审计与 Agentic FinOps。

---

# 0. V7.2 架构收口摘要

V7.2 在 V7.0 完整能力基线上吸收 V7.1 的模块化建议，但不引入新的产品能力域。
...
| 交付标准 | 保持 Scenario A-F；Agent Mesh/Team/Anomaly 不新增 Golden Scenario，先以实验/专项测试验证 |
...
| 档位 | 存储 | 协调 | 推荐用途 | 建议规模 |
|---|---|---|---|---|
| Lite | SQLite | 内存 | 开发、小团队、边缘 | ≤200 RPS |
| Standard | PostgreSQL | Valkey/Redis | 主流生产 | ≤5,000 RPS/副本集 |
| Enterprise | PostgreSQL | Valkey/Redis Cluster | 大规模多租户 | 水平扩展，50k+ RPS |

Enterprise 高量 Analytics 可选 ClickHouse / Kafka-compatible sink / Object Storage，不作为最小依赖。
...
- Project × Department / User / Agent 等交叉分析；
...
6. **Agentic 可治理**：能否回答“哪个用户/Agent 通过怎样的 Delegation Chain 调用了哪个 Agent/Tool、为何允许/拒绝、一个 Task 总共花了多少钱、是否出现异常循环或越权”。
...
V7.0 把 Agentic 能力作为既有 Gateway 治理对象扩展，而不是建立第二套 Agent 平台。
...
- A2A：`1.0.0`，支持 Agent Card Discovery、签名校验、Task/Message/Artifact、Streaming/Push 与协议扩展。

## 1.10 模块化设计原则

V7.2 采用 **Modular Monolith**：
...
└──────────┬───────────┘
           │ RuntimeSnapshot
           │ LISTEN/NOTIFY / PubSub + Poll Fallback
           ▼
┌────────────────────────────────────────────┐
│               Data Plane                   │
│ Admission → Guard → Preflight → Routing    │
│ → Execution → Output Guard → Accounting    │
└────────────┬───────────────────────────────┘
             │
      Models / MCP Tools / A2A Agents / Approval Systems
```
...
V7.0 在 Data Plane 内统一使用 Interaction 语义，不为 Model/MCP/A2A 建立平行 Pipeline。
...
| 数据 | Owner |
|---|---|
| tenants / projects / org_units / memberships | Tenancy / Organization |
| users / role_assignments / service_accounts | Identity / Auth |
| providers / credentials / deployments / logical_models | Model Catalog |
| agents / agent_versions / endpoints / capabilities | Agent Registry |
| mcp_servers / tools | Tool Registry |
| policies / guardrail versions | Policy / Guardrail |
| delegation_grants / approval_requests / agent_tasks | Agentic Governance |
| usage / pricing / budget facts | FinOps |
| request/security/agent/tool events | Observability/Event Store |
| config_drafts / config_versions | Config Plane |
...
type Principal struct {
    ...
    AuthMethod       string // api_key|delegated_jwt|oidc|service_account
    AttributionTrust string // verified|key_bound|delegated|unverified|none
...
- A2A Agent Card 默认从 `/.well-known/agent-card.json` 或管理员指定 URL 发现；存在签名时必须校验，失败按 Tenant Policy 拒绝或标记 untrusted；
...
Agent 是一等 Principal，但不等价于 User。每个 Agent 必须有稳定 `agent_id`，并绑定 Tenant/Project、Owner、Service Account、版本和风险级别。
...
V7.2 不把 `Agent Team` 和 `Agent Mesh` 设为 Core 一级运行对象。
...
- **Application / Agent / Service Account / API Key**：调用主体或资源主体；
...
Model Allowlist
Data Residency
Application / Agent / Service Account / API Key
```
...
```text
Tenant → Project → Application/Agent/API Key
```
...
CREATE TABLE agent_versions (
    ...
    status TEXT NOT NULL
      CHECK (status IN ('draft','active','draining','retired')),
...
Usage Event 是不可变财务事实记录。人员调岗后不得回写历史 `org_unit_id/org_path_snapshot`。
...
CREATE TABLE usage_rollup_hourly (
    ...
    tenant_id UUID NOT NULL,
    dimension_type TEXT NOT NULL, -- project|user|org_unit|application|agent|model|provider
...
Optional ClickHouse / Event Sink
```

Gateway readiness 必须满足：

- 已加载至少一个 active RuntimeSnapshot；
- config_drift 未超过 grace；
- Enterprise strict security mode 下 Security Epoch 未超 staleness。
...
| SLI | Target |
|---|---|
| Standard Data Plane Availability | ≥99.9% |
| Enterprise architecture target | ≥99.95% |
...
V7.2 采用 **Modular Monolith + Contract Boundary**。目录体现职责，不代表独立进程。
...
10. `catalog/agent` 管 Agent 元数据/版本/能力，`agentic` 管 Delegation/Task/Approval，不重复所有权；
11. `pkg/` 只放明确承诺稳定兼容的外部扩展接口，内部 Agent API 不因目录方便而提前公开；
12. Budget/Accounting/Pipeline 不开放第三方覆盖。

## 34.2 架构边界 CI

CI 增加 `architecture-test`：

- Go import graph 检查禁止依赖；
- 禁止 Data Plane import Control Plane ORM；
- 禁止 Protocol Adapter import Domain implementation；
- 禁止跨模块 Repository concrete package；
- 禁止新建无 `TenantScope` 的 Tenant Repository 方法；
- 检查 `pkg/` API breaking changes；
- 检查模块表 Owner manifest 与 migration Owner 一致。
...
从 v7.2 起继续执行“禁止平均建设所有模块”的原则。任何 Epic/Feature 进入开发前必须回答以下问题：

1. 是否直接服务于本章定义的六个真实验证场景之一？
2. 是否强化六个核心能力之一？
...
模拟：User→Agent A→Agent B/Agent C→Tool/Model 的多跳任务，Agent 来自至少两个不同 Runtime/Framework，通过 A2A 与 MCP 交互；混入 Delegation 越权、Agent Loop、Task Budget 超限、高风险 Agent Handoff。
...
六个场景作为跨版本 Golden Scenario，Phase 升级、核心重构和 Release Candidate 必须重新跑通。

## 36.2 跨 Phase Agentic Workstreams

V7.2 不新增 Phase 2.5 / 3.5，避免路线图碎片化。Agentic 通过三个跨 Phase Workstream 推进：
...
- **Scenario F 多智能体协作治理基础闭环通过；**
- 六个 Golden Scenario 在 Enterprise HA/rolling upgrade 后再次通过。
...
67. **产品发布标准以 Scenario A/B/C/D/E/F 端到端验收为核心，不以功能清单完成率作为发布依据；**
68. **V7.2 继续将 Agentic Governance 定义为现有 Gateway Governance 的对象扩展，而不是新建 Agent Runtime；除该扩展外原则上不再新增新的大能力域；**
...
85. **V7.2 采用 Modular Monolith；模块边界是代码/数据所有权边界，不默认拆成微服务，单二进制仍为首选部署。**
...
95. **V7.2 不新增 Phase 2.5/3.5；Agentic Foundation/Observability 采用跨 Phase Workstream，避免版本路线图碎片化。**
...
10. **Golden Scenario**：至少映射一个 Scenario A/B/C/D/E/F 或明确说明为何不需要。
...
```text
v7.0.x   Core correctness / compatibility / Agentic governance foundation
v6.10    当且仅当出现需要跨模块变更的真实生产需求
```

原则：真实运行数据优先于文档推演。新架构决策必须由生产问题、客户场景或明确性能瓶颈驱动。
...
V7.0 Agentic Adapter 以以下公开规范为实现基线，核心数据模型不直接复制协议对象：
