// Package anthropic invokes Anthropic Messages endpoints.
package anthropic
...
type Config struct {
	BaseURL, SecretRef, MessagesPath, APIVersion string
...
func DefaultConfig() Config {
	return Config{MessagesPath: "/v1/messages", APIVersion: "2023-06-01", RequestTimeout: 30 * time.Second, HealthTimeout: 2 * time.Second, Capabilities: contracts.CapabilitySet{"chat": true, "stream": true}}
}
...
func New(config Config, client *http.Client, secrets SecretResolver) (*Connector, error) {
	...
	if err != nil || parsed.Host == "" {
		return nil, errors.New("valid Anthropic base URL is required")
	}
	if parsed.Scheme != "https" && !config.AllowInsecureHTTP {
		return nil, errors.New("Anthropic connector requires HTTPS")
	}
	if config.SecretRef == "" || config.MessagesPath == "" || config.APIVersion == "" || config.RequestTimeout <= 0 || config.HealthTimeout <= 0 || client == nil || secrets == nil {
		return nil, errors.New("complete Anthropic connector configuration is required")
	}
...
func (c *Connector) Invoke(ctx context.Context, request contracts.InvocationRequest) (*contracts.InvocationResponse, error) {
	if request.Request == nil || request.Request.Kind != interaction.RequestChat && request.Request.Kind != interaction.RequestResponses {
		return nil, errors.New("Anthropic connector requires chat request")
	}
	body, err := protocol.EncodeMessages(request.Request)
	if err != nil {
		return nil, err
	}
	response, err := c.do(ctx, body, false)
	if err != nil {
		return nil, err
	}
...
func (c *Connector) Stream(ctx context.Context, request contracts.InvocationRequest, writer contracts.StreamWriter) error {
	if request.Request == nil || request.Request.Kind != interaction.RequestChat && request.Request.Kind != interaction.RequestResponses {
		return errors.New("Anthropic stream requires chat request")
	}
	streamRequest := *request.Request
	streamRequest.Stream = true
	body, err := protocol.EncodeMessages(&streamRequest)
	if err != nil {
		return err
	}
	response, err := c.do(ctx, body, true)
	if err != nil {
		return err
	}
	defer response.Body.Close()
...
func (c *Connector) Health(ctx context.Context, _ contracts.TargetRef) contracts.HealthStatus {
	...
	return contracts.HealthStatus{Healthy: err == nil, CheckedAt: time.Now(), Reason: errorText(err)}
}
...
func (c *Connector) NormalizeError(err error) *contracts.UpstreamError {
	...
	return &contracts.UpstreamError{Code: "upstream_transport_error", Retryable: errors.Is(err, context.DeadlineExceeded) || errors.Is(err, io.ErrUnexpectedEOF), Message: "upstream request failed"}
}
func (c *Connector) do(ctx context.Context, body []byte, stream bool) (*http.Response, error) {
	...
	request, err := http.NewRequestWithContext(callCtx, http.MethodPost, strings.TrimRight(c.config.BaseURL, "/")+c.config.MessagesPath, bytes.NewReader(body))
	if err != nil {
		cancel()
		return nil, err
	}
	request.Header.Set("x-api-key", string(secretCopy))
	request.Header.Set("anthropic-version", c.config.APIVersion)
